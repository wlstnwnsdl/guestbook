...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="jjuna0111/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/wlstnwnsdl/guestbook.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Staging Deploy') {
            steps {
                sshagent(credentials: ['Staging-PrivateKey']) {
                    sh "ssh -o StrictHostKeyChecking=no root@13.125.11.128 docker container rm -f guestbookapp"
                    sh "ssh -o StrictHostKeyChecking=no root@13.125.11.128 docker container run \
                                        -d \
                                        -p 38080:80 \
                                        --name=guestbookapp \
                                        -e MYSQL_IP=15.164.131.7 \
                                        -e MYSQL_PORT=3306 \
...
*/
            slackSend(tokenCredentialId: 'slack-token'
                , channel: '#교육'
                , color: 'good'
                , message: "${JOB_NAME} (${BUILD_NUMBER}) 빌드 끝났습니다. Details: (<${BUILD_URL} | here >)")
        }
        success { 
            slackSend(tokenCredentialId: 'slack-token'
                , channel: '#교육'
